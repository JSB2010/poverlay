...
def normalized_overlay_fps(value: float, round_to_integer: bool) -> float:
    if round_to_integer:
            ...
            with timers.timer("loading timeseries"):
                ...
                if args.use_gpx_only:
                    ...
                    if args.input:
                        ...
                        overlay_frame_rate = normalized_overlay_fps(
                            recording.video.frame_rate(),
                            round_to_integer=args.overlay_fps_round,
                        )
                    ...
                    frame_meta = timeseries_to_framemeta(
                        ...
                        duration=duration,
                        step_seconds=1.0 / overlay_frame_rate,
                        packets_per_second=max(1, int(round(overlay_frame_rate))),
                    )
                    ...
                    overlay_frame_rate = normalized_overlay_fps(
                        gopro.recording.video.frame_rate(),
                        round_to_integer=args.overlay_fps_round,
                    )
                    packets_per_second = frame_meta.packets_per_second()
                ...
                if args.overlay_fps:
                    overlay_frame_rate = normalized_overlay_fps(args.overlay_fps, round_to_integer=False)
            ...
            with MapRenderer(
                ...
                if generate == "none":
                    ...
                    ffmpeg = FFMPEGOverlay(
                        ...
                        overlay_framerate=overlay_frame_rate,
                        execution=execution,
                ...
                # Draw an overlay frame at metadata cadence (usually aligned to video fps in GPX-only mode).
                timelapse_correction = frame_meta.duration() / video_duration
                log(f"Timelapse Factor = {timelapse_correction:.3f}")
                step_seconds = (1.0 / max(overlay_frame_rate, 1.0)) * timelapse_correction
