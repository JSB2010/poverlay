...
def gopro_dashboard_arguments(args=None):
    ...
    render.add_argument(
        "--overlay-fps",
        type=float,
        help="Override overlay framerate. By default it matches source video fps when input video is provided.",
    )
    render.add_argument(
        "--overlay-fps-round",
        action="store_true",
        help="Round detected source video fps to nearest integer (e.g. 29.97 -> 30).",
    )
    ...
    if args.overlay_fps is not None and args.overlay_fps <= 0:
        quit("--overlay-fps must be > 0")
