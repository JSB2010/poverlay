  ...
  return (
    <main className="page-shell">
      <header className="hero-panel">
        <div>
          <p className="eyebrow">POVerlay</p>
          <h1>GoPro Telemetry Overlay Studio</h1>
        ...
        <div className="hero-badges">
          ...
          <span>Multi-clip batch renders</span>
          <span>VPS deploy-ready</span>
        </div>
      </header>

      <div className="workspace-grid">
        <section className="config-panel panel-card">
          <form onSubmit={handleSubmit}>
            <div className="section-title-row">
              <h2>Upload</h2>
              <p>Start with one GPX file and one or more clips.</p>
            </div>

            <div className="field-grid two-col">
              <label className="field-block" htmlFor="gpx">
                <span>GPX file (Slopes export)</span>
                <input
                  id="gpx"
                  type="file"
                  accept=".gpx"
              ...
              <label className="field-block" htmlFor="videos">
                <span>GoPro clips</span>
                <input
                  id="videos"
                  type="file"
            ...
            <div className="section-title-row">
              ...
              <p>Main controls for look, layout, and export behavior.</p>
            </div>

            <div className="field-grid three-col">
              <label className="field-block" htmlFor="overlay_theme">
              ...
              <label className="field-block" htmlFor="layout_style">
              ...
              <label className="field-block" htmlFor="render_profile">
                <span>Export codec</span>
                <select
                  id="render_profile"
              ...
              <label className="field-block" htmlFor="units_preset">
                <span>Units preset</span>
                <select
                  id="units_preset"
              ...
              <label className="field-block" htmlFor="fps_mode">
                <span>FPS strategy</span>
                <select
                  id="fps_mode"
              ...
              <label className="field-block" htmlFor="fixed_fps">
                <span>Fixed FPS value</span>
                <input
                  id="fixed_fps"
                  type="number"
                  min="1"
              ...
              <label className="field-block" htmlFor="gpx_offset_seconds">
                <span>GPX time offset (seconds)</span>
                <input
            ...
            <div className="preview-grid">
              <article className="preview-card">
                ...
                <div className="theme-preview-swatches">
                  <span style={{ background: panelColor }}>Panel</span>
                  <span style={{ background: panelAltColor }}>Panel Alt</span>
                  <span style={{ background: speedColor }}>Speed</span>
                  <span style={{ background: accentColor }}>Accent</span>
                  <span style={{ background: textColor, color: "#102742" }}>Text</span>
                </div>
                ...
                >
                  <div className="mini-speed">
                    <small style={{ color: accentColor }}>SPEED</small>
                    <strong style={{ color: speedColor }}>62</strong>
                  </div>
                  <div className="mini-stats" style={{ color: textColor }}>
                    <span>ALT 2100</span>
                    <span>GRADE 8.2%</span>
                    <span>DIST 4.6</span>
                  </div>
              ...
              <article className="preview-card">
                <h3>Layout Previews</h3>
                <div className="layout-grid">
                  {layoutStyles.map((layoutStyle) => (
                    <button
                      key={layoutStyle.id}
                      type="button"
                      className={`layout-tile ${layoutStyle.id === formState.layout_style ? "selected" : ""}`}
                    ...
                    >
                      ...
                      <div className="layout-meta">
                        <strong>{layoutStyle.label}</strong>
                        <small>{layoutStyle.description}</small>
                      </div>
                    </button>
                  ))}
                </div>
            ...
            <div className="section-title-row">
              ...
              <p>Choose exactly what appears in the rendered overlay.</p>
            </div>
            ...
            <div className="component-grid">
              {componentOptions.map((option) => {
                ...
                return (
                  <label key={option.id} className={`component-toggle ${isDisabled ? "disabled" : ""}`}>
                    <input
                      type="checkbox"
                      checked={checked}
                      disabled={isDisabled}
            ...
            <details className="advanced-panel">
              ...
              <div className="field-grid three-col advanced-grid">
                <label className="field-block" htmlFor="map_style">
                  <span>Map style</span>
                  <select
                    id="map_style"
                ...
                <label className="field-block" htmlFor="speed_units">
                ...
                <label className="field-block" htmlFor="gpx_speed_unit">
                  <span>GPX speed source units</span>
                  <select
                    id="gpx_speed_unit"
                ...
                <label className="field-block" htmlFor="distance_units">
                ...
                <label className="field-block" htmlFor="altitude_units">
                ...
                <label className="field-block" htmlFor="temperature_units">
        ...
        <aside className="status-column">
          <section className="panel-card status-card">
            <div className="status-header">
              <div>
                <h2>Render Status</h2>
                <p>{job?.message ?? (activeJobId ? "Waiting for updates..." : "Run a render to view progress.")}</p>
              </div>
              {job?.status && <span className={`status-pill ${job.status}`}>{job.status.replaceAll("_", " ")}</span>}
            </div>

            <div className="progress-shell" role="progressbar" aria-valuenow={job?.progress ?? 0} aria-valuemin={0} aria-valuemax={100}>
            ...
            <ul className="video-list">
              {(job?.videos ?? []).map((video) => {
                ...
                return (
                  <li key={video.input_name}>
                    <div className="video-row-top">
                      <strong>{video.input_name}</strong>
                      <span className={`status-pill ${video.status}`}>
                        {video.status} {video.progress ?? 0}%
                      </span>
                    </div>

                    {clipDetails.length > 0 && <p>{clipDetails.join(" • ")}</p>}

                    <div className="row-actions">
                      {video.download_url && (
                        <a href={`${API_BASE}${video.download_url}`}>
                          Download video
                        </a>
                      )}

                      {video.log_name && job?.id && (
                        <a href={`${API_BASE}/api/jobs/${job.id}/log/${video.log_name}`}>
                          Renderer log
                        </a>
                      )}
                    </div>
            ...
            {job?.download_all_url && (
              <a className="secondary-btn" href={`${API_BASE}${job.download_all_url}`}>
          ...
          <section className="panel-card architecture-card">
            <h2>POVerlay Stack</h2>
            <ul>
              <li>Frontend: Next.js App Router with TypeScript</li>
              <li>API: FastAPI job orchestration and render queue</li>
              <li>Renderer: gopro-dashboard-overlay + ffmpeg</li>
              <li>Deploy model: Next.js + FastAPI behind Nginx</li>
            </ul>
          </section>
        </aside>
      </div>
